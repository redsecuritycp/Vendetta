...
ADVERTENCIA: este motor es OPT-IN — NO forma parte del pipeline de
`FullScanner` y NO se ejecuta desde `POST /api/scan`. Solo invocar
manualmente contra sistemas propios o con autorización explícita.
"""
...
def analyze(url: str, max_concurrent: int = 600, duration_seconds: int = 30) -> Dict:
    """
    Helper de conveniencia: instancia LoadTestEngine, lanza el test,
    espera a que termine (bloqueante) y devuelve las stats finales.
    ...
    Args:
        url: URL objetivo
        max_concurrent: Máximo de requests concurrentes (default 600)
        duration_seconds: Duración en segundos (default 30, 0 = ilimitado NO recomendado)

    Returns:
        Dict con las stats finales (mismo formato que LoadTestEngine.get_stats()).
    """
    engine = LoadTestEngine()
    if duration_seconds <= 0:
        # No permitir duración ilimitada en el helper bloqueante
        duration_seconds = 30
    engine.start_test(url, max_concurrent=max_concurrent, duration_seconds=duration_seconds)

    # Esperar a que termine el thread
    if engine.test_thread:
        engine.test_thread.join(timeout=duration_seconds + 10)
...
def main():
    ...
    print("=" * 60)
    print("LOAD TEST ENGINE - Prueba de Carga HTTP")
    print("=" * 60)
    print("\nADVERTENCIA: Solo para sistemas propios o autorizados")
    print("=" * 60)

    if len(sys.argv) < 2:
        print("\nUso: python -m modules.scanners.load_test.scanner <url> [concurrent] [duracion]")
        print("Ejemplo: python -m modules.scanners.load_test.scanner https://mi-sitio.com 600 30")
        sys.exit(1)

    url = sys.argv[1]
    concurrent = int(sys.argv[2]) if len(sys.argv) > 2 else 600
    duration = int(sys.argv[3]) if len(sys.argv) > 3 else 30

    stats = analyze(url, max_concurrent=concurrent, duration_seconds=duration)

    print(f"\n=== RESULTADO ===")
    print(f"Requests enviados: {stats['total_sent']}")
    print(f"Requests completados: {stats['total_completed']}")
    print(f"Éxitos (200 OK): {stats['successes']}")
    print(f"Errores/Timeouts: {stats['errors']}")
    print(f"Latencia avg: {stats['latency_avg']:.3f}s")
    print(f"Latencia min: {stats['latency_min']:.3f}s")
    print(f"Latencia max: {stats['latency_max']:.3f}s")
    print(f"RPS aproximado: {stats['rps']:.1f}")
