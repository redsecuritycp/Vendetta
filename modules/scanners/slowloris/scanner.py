...
def analyze(url: str, socket_count: int = 200, duration: int = 30) -> SlowlorisResult:
    """
    Helper de conveniencia: instancia SlowlorisAttacker y corre analyze().
    ...
    return attacker.analyze(url, socket_count=socket_count, duration=duration)
...
def main():
    ...
    if len(sys.argv) < 2:
        print("\nUso: python -m modules.scanners.slowloris.scanner <url> [sockets] [duracion]")
        print("Ejemplo: python -m modules.scanners.slowloris.scanner https://mi-sitio.com 200 30")
        sys.exit(1)
